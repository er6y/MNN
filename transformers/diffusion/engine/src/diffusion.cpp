...
VARP Diffusion::unet(VARP text_embeddings, int iterNum, int randomSeed, float cfgScale, std::function<void(int)> progressCallback) {
    ...
    for (int i = 0; i < mTimeSteps.size(); i++) {
        ...
        if (mModelType == STABLE_DIFFUSION_ZIMAGE) {
            ...
            // Copy latent data into mSampleVar buffer using MNN's input() method
            // This is more efficient than readMap/writeMap as it can be done on GPU
            ...
            // Update plms in-place to avoid creating new VARP each step
            // This is critical for GPU memory management - creating new VARP each step
            // causes memory accumulation and eventually OOM or GPU hang
            {
                auto plmsPtr = plms->writeMap<float>();
                auto noisePtr = noise_pred->readMap<float>();
                if (plmsPtr && noisePtr) {
                    int latentSize = mLatentC * mLatentH * mLatentW;
                    for (int j = 0; j < latentSize; ++j) {
                        plmsPtr[j] += dt * noisePtr[j];
                    }
                }
            }
            // Note: output/noise_pred/outputs are local variables, they will be
